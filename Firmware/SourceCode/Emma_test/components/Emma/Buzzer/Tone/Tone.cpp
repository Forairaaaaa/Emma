...
static void tone_task(void*){
  ...
  while(1){
    ...
    switch(tone_msg.tone_cmd){
      case TONE_START:
        ESP_LOGD(TAG, "Task received from queue TONE_START: _pin=%d, frequency=%u Hz, duration=%lu ms", tone_msg.pin, tone_msg.frequency, tone_msg.duration);

        ESP_LOGD(TAG, "Setup LED controll on channel %d", _channel);
      ...
      case TONE_END:
        ESP_LOGD(TAG, "Task received from queue TONE_END: pin=%d", tone_msg.pin);
        // ledcDetachPin(tone_msg.pin);
        ledcWriteTone(_channel, 0);
        break;

      case TONE_SET_CHANNEL:
        ESP_LOGD(TAG, "Task received from queue TONE_SET_CHANNEL: channel=%d", tone_msg.channel);
...
static int tone_init(){
  if(_tone_queue == NULL){
    ESP_LOGV(TAG, "Creating tone queue");
    _tone_queue = xQueueCreate(128, sizeof(tone_msg_t));
    if(_tone_queue == NULL){
      ESP_LOGE(TAG, "Could not create tone queue");
      return 0; // ERR
    }
    ESP_LOGV(TAG, "Tone queue created");
  }

  if(_tone_task == NULL){
    ESP_LOGV(TAG, "Creating tone task");
    xTaskCreate(
    ...
    ESP_LOGV(TAG, "Tone task created");
  }
  return 1; // OK
}
